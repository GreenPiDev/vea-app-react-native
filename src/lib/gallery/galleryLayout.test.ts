// Mirrors vea-frontend/src/components/3d/galleryLayout.test.ts (same pure
// geometry logic, ported from Vitest to Jest globals).
import {
  buildRoomLayout,
  EYE_HEIGHT,
  WALL_THICKNESS,
...
describe('placeArtworksAlongWall', () => {
  ...
  it('splits a wall into N equal slots and centers each artwork in its own slot', () => {
    ...
    expect(placements[1].position[0]).toBeCloseTo(2.5);
  });
