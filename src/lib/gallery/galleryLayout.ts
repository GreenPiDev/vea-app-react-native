// Byte-for-byte mirror of vea-frontend/src/components/3d/galleryLayout.ts —
// pure geometry/math, no React/Three/RN dependency, so it copies verbatim.
// Keep the two files in sync by hand (same cross-repo caveat as
// SOCKET_EVENTS/i18n JSON) — this is the single source of truth for building
// geometry on THIS platform, mirroring the single-source-of-truth role the
// original plays on web.
...
// Used by lib/gallery/backendAdapter.ts to place backend-linked artworks on
// a wall run.
